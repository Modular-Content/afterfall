...
-- пару слов про ULX/ULib
--[[
	поддержка юлкса как и юлиба это конечно смелое решение со стороны разработчиков фиксед клокворка, но лично я НЕ РЕКОМЕНДУЮ ЕГО ИСПОЛЬЗОВАТЬ
	в юлксе как и в юлибе очень много эксплойтов (например есть эксплойт, который позволяет полностью крашнуть сервер)
	так что не используйте никогда юлкс/юлиб, даже сервергуард лучше будет, чем эти говнокодные админки
	говорю на опыте работы с юлксом/юлибом, так что не надо мне говорить, что я не прав, потому что я знаю о чем говорю
--]]
...
function Clockwork.kernel:Serialize(tableToSerialize)
	local wasSuccess, value = xpcall(pon.encode, debug.traceback, tableToSerialize)
...
function Clockwork.kernel:Deserialize(stringToDeserialize)
	local wasSuccess, value = xpcall(pon.decode, debug.traceback, stringToDeserialize)
...
if SERVER then
	...
	function Clockwork.kernel:PerformDateTimeThink()
		...
		self:SetNetVar("Minute", Clockwork.time:GetMinute())
		self:SetNetVar("Hour", Clockwork.time:GetHour())
		self:SetNetVar("Date", day .. "/" .. month .. "/" .. Clockwork.date:GetYear())
		self:SetNetVar("Day", Clockwork.time:GetDay())
	end
	...
	function Clockwork.kernel:DrawBars(info, class)
		if Clockwork.plugin:Call("PlayerCanSeeBars", class) then
			...
			Clockwork.option:SetFont("bar_text", Clockwork.option:GetFont("auto_bar_text"))
				for k, v in ipairs(Clockwork.bars.stored) do
	...
	function Clockwork.kernel:DrawBar(x, y, width, height, color, text, value, maximum, flash, barInfo)
		...
		backgroundColor.a = barInfo.color.a
		foregroundColor.a = barInfo.color.a
	...
	function Clockwork.kernel:PrintColoredText(...)
		...
		for k, v in pairs({...}) do
			if type(v) == "Player" then
				text[#text + 1] = v:GetTeamClass().color
				text[#text + 1] = v:Name()
	...
	--[[
		@codebase Shared
		@details A function to draw the health and armor bar.
		@returns {Unknown}
	--]]
	function Clockwork.kernel:DrawHealthArmorBar()
		...
		hudHealth = math.Approach(hudHealth, health, 1)
		hudArmor = math.Approach(hudArmor, armor, 1)

		if health > 0 then
			Clockwork.bars:Add("HEALTH", 10, Color(179, 46, 49, 255), "Здоровье", hudHealth, Clockwork.Client:GetMaxHealth(), hudHealth < 10)
		end
		if armor > 0 then
			Clockwork.bars:Add("ARMOR", 9, Color(139, 174, 179, 255), "Броня", hudArmor, Clockwork.Client:GetMaxArmor(), hudHealth < 10)
		end
	end
	...
	function Clockwork.kernel:DrawCinematicInfo()
		...
		local cinematicInfo = Schema:GetCinematicIntroInfo()
		local colorWhite = Clockwork.option:GetColor("white")
		local colorInfo = Clockwork.option:GetColor("information")
...
function Clockwork.kernel:HasObjectAccess(player, object)
	...
	if object.ownFaction then
		if object.ownFaction == faction then
			hasAccess = true
		end
	end
