...
function Clockwork:PlayerThink(player, curTime, infoTable)
	...
	if cwPlugin:Call('PlayerShouldSmoothSprint', player, infoTable) then
		local target = player:IsRunning() and infoTable.runSpeed or infoTable.walkSpeed
	    player.cwLastRunSpeed = player.cwLastRunSpeed or infoTable.walkSpeed
	    local accel = player:IsRunning() and 8 or 6
	    player.cwLastRunSpeed = Lerp(FrameTime() * accel, player.cwLastRunSpeed, target)
	...
	player:SetNetVar("IsRunMode", infoTable.isRunning)
...
--[[
	...
	@params {Unknown} Missing description for player.
	@params {Unknown} Missing description for infoTable.
	@returns {Unknown}
--]]
...
function Clockwork:PlayerFlagsGiven(player, flags)
	...
	player:SetNetVar("Flags", player:GetFlags())
end
...
function Clockwork:PlayerInitialSpawn(player)
	...
	player.cwNetVars = player.cwNetVars or {}
...
--[[
	...
	@returns {Unknown}
--]]
function Clockwork:PlayerInfoSent(player)
	if player:IsBot() then
...
--[[
	@codebase Server
	@details Called when a player's info stream should be sent.
	@param {Unknown} Missing description for player.
	@returns {Unknown}
--]]
function Clockwork:PlayerSendInfoStream(player)
...
function Clockwork:PlayerPlayDeathSound(player, gender)
	return 'ambient/voices/' .. (string.lower(gender) == 'female' and 'f' or 'm') .. '_scream1.wav'
end
...
function Clockwork:Tick()
	...
	for k, v in pairs(plyTable) do
		if v:HasInitialized() then
			...
			if not v.cwNextSetNetVars then
				v.cwNextSetNetVars = curTime + 1
			end

			if curTime >= v.cwNextThink then
				cwPly:CallThinkHook(v, curTime >= v.cwNextSetNetVars, curTime)
			end
		end
	end
...
--[[
	...
	@returns {Unknown}
--]]
function Clockwork:PlayerSetNetVars(player, curTime)
...
function Clockwork:PlayerCanSwitchCharacter(player, character)
	if not player:Alive() and not player:IsCharacterMenuReset() and not player:GetNetVar("CharBanned") then
...
function Clockwork:PlayerRagdolled(player, state, ragdoll)
	player:SetNetVar("FallenOver", false)
end
...
function Clockwork:PlayerCharacterLoaded(player)
	player:SetNetVar("InvWeight", cwConfig:Get("default_inv_weight"):Get())
	player:SetNetVar("InvSpace", cwConfig:Get("default_inv_space"):Get())
...
function Clockwork:PlayerConfigInitialized(player)
	cwPlugin:Call("PlayerSendInfoStream", player)

	if not player:IsBot() then
		timer.Simple(FrameTime() * 32, function()
			if IsValid(player) then
				netstream.Start(player, "InfoStreaming", true)
			end
		end)
	else
		cwPlugin:Call("PlayerInfoSent", player)
	end
end
...
-- GetTargetRecognises netstream callback.
netstream.Hook("GetTargetRecognises", function(player, data)
	if IsValid(data) and data:IsPlayer() then
		player:SetNetVar("TargetKnows", cwPly:DoesRecognise(data, player))
	end
end)

-- EntityMenuOption netstream callback.
...
-- InfoSent netstream callback.
netstream.Hook("InfoSent", function(player, data)
	if not player.cwInfoSent then
		cwPlugin:Call("PlayerInfoSent", player)

		timer.Simple(FrameTime() * 32, function()
			if IsValid(player) then
				netstream.Start(player, "InfoStreamed", true)
			end
		end)

		player.cwInfoSent = true
	end
end)
...
-- InteractCharacter netstream callback.
...
-- GetQuizStatus netstream callback.
...
-- QuizCompleted netstream callback.
...
-- UnequipItem netstream callback.
