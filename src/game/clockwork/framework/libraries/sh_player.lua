...
--]]
	function Clockwork.player:GetMaxWeight()
		...
		local weight = Clockwork.Client:GetNetVar("InvWeight") or Clockwork.config:Get("default_inv_weight"):Get()
...
--]]
	function Clockwork.player:GetMaxSpace()
		local itemsList = Clockwork.inventory:GetAsItemsList(Clockwork.inventory:GetClient())
		local space = Clockwork.Client:GetNetVar("InvSpace") or Clockwork.config:Get("default_inv_space"):Get()
...
--]]
	...
	@returns {Unknown}
--]]
	function Clockwork.player:HasInfoStreamed()
		return Clockwork.InfoHasStreamed
	end
...
--]]
	function Clockwork.player:GetAction(player, percentage)
		local startActionTime = player:GetNetVar("StartActTime")
		local actionDuration = player:GetNetVar("ActDuration")
		local curTime = CurTime()
		local action = player:GetNetVar("ActName")
...
--]]
	function Clockwork.player:GetWeaponRaised(player)
		return player:GetNetVar("IsWepRaised")
	end
...
--]]
	function Clockwork.player:IsRagdolled(player, exception, entityless)
		if player:GetRagdollEntity() or entityless then
			if player:GetNetVar("IsRagdoll") == 0 then
				return false
			elseif player:GetNetVar("IsRagdoll") == exception then
				return false
			else
				return player:GetNetVar("IsRagdoll") ~= RAGDOLL_NONE
			end
		end
	end
...
--]]
	function Clockwork.player:GetCharacterKey(player)
		if IsValid(player) then return player:GetNetVar("Key") end
	end
...
--]]
	function Clockwork.player:GetRagdollState(player)
		if player:GetNetVar("IsRagdoll") == 0 then
			...
			return player:GetNetVar("IsRagdoll")
		end
	end
...
--]]
	function Clockwork.player:GetPhysDesc(player)
		...
		local physDesc = player:GetNetVar("PhysDesc")
...
--]]
	function Clockwork.player:GetWages()
		return Clockwork.Client:GetNetVar("Wages", 0)
	end
...
--]]
	function Clockwork.player:GetCash()
		return Clockwork.Client:GetNetVar("Cash")
	end
...
--]]
	function Clockwork.player:HasAnyFlags(player, flags, byDefault)
		local playerFlags = player:GetNetVar("Flags")
...
--]]
	function Clockwork.player:GetDrunk()
		local isDrunk = Clockwork.Client:GetNetVar("IsDrunk")
				...
				queryObj:SetCallback(function(result)
					...
					if cwDatabase:IsResult(result) then
						...
						self:LoadCharacter(player, characterID, {
							...
							netstream.Start(player, "CharacterFinish", {
								wasSuccess = true
							})
...
--]]
	function Clockwork.player:SetCharacterMenuOpen(player, shouldReset)
		if player:HasInitialized() then
			netstream.Start(player, "CharacterOpen", shouldReset == true)
...
--]]
	function Clockwork.player:UpdateWeaponRaised(player)
		...
		player:SetNetVar("IsWepRaised", isRaised)
...
--]]
	function Clockwork.player:GetWeaponRaised(player, bIsCached)
		if bIsCached then return player:GetNetVar("IsWepRaised") end
		local weapon = player:GetActiveWeapon()
...
--]]
	function Clockwork.player:SetWhitelisted(player, faction, isWhitelisted)
		...
		netstream.Start(player, "SetWhitelisted", {faction, isWhitelisted})
	end
...
--]]
	function Clockwork.player:Notify(player, text, class, icon)
		...
		elseif not class then
			...
			netstream.Start(player, "Notification", {
				text = text,
				class = class
			})
		end
...
--]]
	function Clockwork.player:IsRagdolled(player, exception, bNoEntity)
		if player:GetRagdollEntity() or bNoEntity then
			local ragdolled = player:GetNetVar("IsRagdoll")
...
--]]
	function Clockwork.player:LoadCharacter(player, characterID, mergeCreate, Callback, shouldForce)
		...
		if mergeCreate then
			...
			if character then
				...
				if self:SetBasicNetVars(player) then
	...
	@returns {Unknown}
--]]
	function Clockwork.player:SetBasicNetVars(player)
		local gender = player:GetGender()
		local faction = player:GetFaction()
		local factionList = cwFaction:GetAll()

		if factionList[faction] then
			player:SetNetVar("Faction", factionList[faction].index)
		end
...
--]]
	...
	@param {Unknown} Missing description for player.
	@param {Unknown} Missing description for setNetVars.
	@param {Unknown} Missing description for curTime.
	@returns {Unknown}
--]]
	function Clockwork.player:CallThinkHook(player, setNetVars, curTime)
		...
		if setNetVars then
			cwPlugin:Call("PlayerSetNetVars", player, curTime)
			player.cwNextSetNetVars = nil
		end
