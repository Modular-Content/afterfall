...
function Clockwork.class:AssignToDefault(player)
	...
	for k, v in pairs(self.stored) do
		if v.ownFaction and v.isDefault and v.ownFaction == faction then
			defaults[#defaults + 1] = v.name
		end
	end
	...
	if #defaults > 0 then
		...
		for k, v in pairs(self.stored) do
			if Clockwork.kernel:HasObjectAccess(player, v) then return self:Set(player, v.name) end
		end
	end
end
...
if SERVER then
	function Clockwork.class:Set(player, name, noRespawn, addDelay, model)
		...
		local factionTable = Clockwork.faction:FindByID(newClass.ownFaction)

		if newClass and factionTable then
			player:SetTeam(factionTable.team)
			player:SetNetVar('TeamClass', name)
			...
			Clockwork.plugin:Call("PlayerClassSet", player, newClass, oldClass, noRespawn, addDelay, model)
