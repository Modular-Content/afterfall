...
function playerMeta:RemoveAccessory(itemTable)
	...
	netstream.Start(self, "RemoveAccessory", {
		itemID = itemID
	})
...
function playerMeta:WearAccessory(itemTable)
	...
	netstream.Start(self, "AddAccessory", {
		itemID = itemID,
		uniqueID = uniqueID
	})
...
function playerMeta:SetForcedAnimation(animation, delay, OnAnimate, OnFinish)
	...
	if bShouldPlay then
		...
		netstream.Start(nil, "ForcedAnimResetCycle", {
			player = self
		})
...
function playerMeta:GetMaxWeight()
	...
	local weight = self:GetNetVar("InvWeight")
...
function playerMeta:GetMaxSpace()
	local itemsList = cwInventory:GetAsItemsList(self:GetInventory())
	local space = self:GetNetVar("InvSpace")
...
function playerMeta:GiveItem(itemTable, isForced)
	...
	if self:CanHoldWeight(itemTable("weight")) and self:CanHoldSpace(itemTable("space")) or isForced then
		...
		netstream.Start(self, "InvGive", cwItem:GetDefinition(itemTable, true))
...
function playerMeta:TakeItem(itemTable)
	...
	netstream.Start(self, "InvTake", {itemTable("uniqueID"), itemTable("itemID")})
...
function playerMeta:SetCharModel()
	Clockwork:PlayerSetModel(self)
end
