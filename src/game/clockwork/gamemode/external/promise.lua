...
-- у меня конечно есть JS/Promise A+, но давать в публичный репозиторий код я не буду
-- так что я оставлю здесь вот этот JS/Promise F-, я не знаю какой жопой его модифицировали... да и что тут модифицировали та вообще...
...
function Promise:_Reject(err)
	...
	if not self._hasRejectionHandler then
		ErrorNoHalt("Unhandled promise rejection: " .. tostring(err) .. "\n")
	end
end

function Promise:_SetHandled()
	self._hasRejectionHandler = true

	if self._parent then
		self._parent:_SetHandled()
	end
end
...
function Promise:Then(onresolve, onreject)
	local child = Promise(function(resolve, reject)
